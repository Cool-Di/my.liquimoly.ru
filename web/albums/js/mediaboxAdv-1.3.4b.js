...
(function() {
	...
	// DOM elements
	overlay, center, image, bottom, captionSplit, title, caption, prevLink, number, nextLink, downLink,
	...
	window.addEvent("domready", function() {
		...
		bottom = new Element("div", {id: "mbBottom"}).injectInside(center).adopt(
			...
			downLink = new Element("a", {id: "mbDownLink", href: "#"}).addEvent("click", download),
	...
	Mediabox = {
		...
		open: function(_images, startImage, _options) {
			options = $extend({
				text: ['<big>&laquo;</big>','<big>&raquo;</big>','<big>&times;</big>', '<span>Скачать</span>'],		// Set "previous", "next", and "close" button content (HTML code should be written as entity codes or properly escaped)
...
//			Vimeo options
			...
			downLink.set('html', options.text[3]);
...
//			if (options.overflow) image.setStyles({overflow: options.overflow});
	...
	function download(e) {
		e.preventDefault();
		downloadFile($$('#mbImage img').get('src')[0]);
	}
