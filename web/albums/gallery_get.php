...
function getfirstImage( $dirname )
    ...
    $ext           = array( "jpg",
                            ...
                            "GIF",
                            "JPEG" );
    $priority_name = "gallery_preview";
    if ( $handle = opendir( $dirname ) )
        {
        //Ищем специальный файл для тумбы
        while ( false !== ( $file = readdir( $handle ) ) )
            {
            $lastdot    = strrpos( $file, '.' );
            $extension  = substr( $file, $lastdot + 1 );
            $withoutExt = preg_replace( '/\\.[^.\\s]{3,4}$/', '', $file );
            if ( $withoutExt == $priority_name && in_array( $extension, $ext ) )
                {
                break;
                }
            }
        //Если превьюхи нет, ищем первую картинку и делаем тумбу из нее
        rewind( $handle );
        if ( !strstr( $file, $priority_name ) )
            {
            while ( false !== ( $file = readdir( $handle ) ) )
                {
                $lastdot   = strrpos( $file, '.' );
...
function get_gallery( $dir )
    {
    if ( empty( $dir ) )
        $dir = $_REQUEST[ "dir" ];
    if ( !defined( "GALLERY_ROOT" ) )
        define( "GALLERY_ROOT", __DIR__ . DIRECTORY_SEPARATOR );
                ...
                if ( $file != "." && $file != ".." )
                    ...
                    if ( file_exists( "$currentdir/" . $file . "/folder.jpg" ) )
                        ...
                        $dirs[] = array( "name" => $file,
                                         ...
                                         "url" => "dir=" . ltrim( $dir . "/" . $file, "/" ),
                                         "src" => "/albums/createthumb.php?filename=$currentdir/" . $file . "/folder.jpg&amp;size=$thumb_size" );
                        }
                    else
                        {
                        ...
                        if ( $firstimage != "" )
                            ...
                            $dirs[] = array( "name" => $file,
                                             ...
                                             "url" => "?dir=" . ltrim( $dir . "/" . $file, "/" ),
                                             "src" => "/albums/createthumb.php?filename=$thumbdir/" . $file . "/" . $firstimage . "&amp;size=$thumb_size" );
                            }
                        else
                            {
                            ...
                            $dirs[] = array( "name" => $file,
                                             "date" => filemtime( $currentdir . "/" . $file ),
                                             "url" => "?dir=" . ltrim( $dir . "/" . $file, "/" ),
                                             "src" => "/albums/images/folder_" . strtolower( $folder_color ) . ".png" );
                            }
                        }
                    }
                }
...
// 3. LOAD FILES
            if ( $file != "." && $file != ".." && $file != "folder.jpg" && !strstr( $file, 'gallery_preview' ) )
                {
                // JPG, GIF and PNG
